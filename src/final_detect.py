...
# read model cheating
def initialize_interpreter(model_path="./model/model_recognize_v5_lite.tflite"):
            ...
            if results.pose_landmarks:
                ...
                if (len(lm_list_0) >= n_time_steps):
                    ...
                    if label_0 == "NORMAL" and pre_label_0 == "SHOPLIFTING":
                        try:
                            ...
                            frames = list(frame_buffer)[-(FRAME_AGO + n_time_steps + sensitivity_0 -1):]
                            print("frame length : ", len(frames))
                            report = await fetch_detection_report(CAMERAID, int(timestamp - len(frames)/FRAME_RATE), int(timestamp), sensitivity_0,80)
                            sensitivity_0 = 0
                            shoplifting_continous_count_0 =0
                    ...
                    if label_0 == "NORMAL" and len(frame_buffer) > (FRAME_AGO + n_time_steps +sensitivity_1 -1):
                        frame_buffer = list(frame_buffer)[-(FRAME_AGO + n_time_steps + sensitivity_1 -1):]
                    lm_list_0.pop(0)
                ...
                if (len(lm_list_1) >= n_time_steps):
                    ...
                    if label_1 == "NORMAL" and pre_label_1 == "SHOPLIFTING":
                        try:
                            print("sensitivity_0  : ", sensitivity_0,"sensitivity_1: ",sensitivity_1)
                            frames = list(frame_buffer)[-(FRAME_AGO + n_time_steps + sensitivity_1 -1):]
                            print("frame length : ", len(frames))
                            report = await fetch_detection_report(CAMERAID, int(timestamp - len(frames)/FRAME_RATE), int(timestamp), sensitivity_1,80)
                            sensitivity_1 =0
                            shoplifting_continous_count_1 =0
                            action_id = report.get('actionId', "0")
                            if action_id != "0":
                    ...
                    #lưu 32 frame trước đó - done
                    if label_1 == "NORMAL" and len(frame_buffer) > (FRAME_AGO + n_time_steps+sensitivity_0-1):
                        frame_buffer = list(frame_buffer)[-(FRAME_AGO + n_time_steps + sensitivity_0-1):]
                    lm_list_1.pop(0)
            else:
                lm_list_0 = []
                lm_list_1 = []
